...
async function run() {
  console.log('🌱 Creating table and seeding features...\n');
  
  // Create table via SQL RPC call
  console.log('📋 Creating features table...');
  const { error: createError } = await supabase.rpc('create_features_table', {}, { 
    count: 'estimated'
  }).then(() => ({ error: null })).catch(e => ({ error: e }));
  
  // Alternative: Just try inserting - Supabase might auto-create the table
  // Actually, let's use raw SQL via the SQL editor endpoint
  ...
  if (error) {
    ...
    if (error.code === 'PGRST205') {
      ...
      console.error('Please create it manually in Supabase console:');
      console.error('https://app.supabase.com/project/hqfszlxdkvwlvpwqqmbd/sql\n');
  ...
  console.log(`📊 Total features in database: ${count}`);
  console.log('\n🎉 Done! Features are now in Supabase.');
}

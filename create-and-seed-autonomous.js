...
async function run() {
  try {
    ...
    for (const feature of MVP_FEATURES) {
      await client.query(
        `INSERT INTO public.features (id, title, description, category, person, column_name, timestamp)
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         ON CONFLICT (id) DO NOTHING`,
        [feature.id, feature.title, feature.description, feature.category, feature.person, feature.column_name, feature.timestamp]
      );
    }
    console.log('✅ All 34 features inserted!\n');

    const result = await client.query('SELECT COUNT(*) FROM public.features');
    const count = result.rows[0].count;
    console.log(`📊 Total features in database: ${count}`);
    console.log('🎉 Done! MVP board is now fully seeded.');
